...
def install_docker():
    """Install docker."""
    if (
        subprocess.call(["which", "docker"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        == 0
    ):
        return
...
def install_golang():
    ...
    if sys.platform == "darwin":
        ...
        subprocess.run(
            ["sudo", "tar", "-C", "/usr/local", "-xzf", "go1.24.4.linux-amd64.tar.gz"], check=True
        )
        subprocess.run(["sudo", "mv", "/usr/local/go", "/usr/local/go1.24.4"], check=True)
        subprocess.run(
            ["sudo", "ln", "-s", "/usr/local/go1.24.4/bin/go", "/usr/local/bin/go"], check=True
        )
...
def install_build_dependencies():
    ...
    if (
        subprocess.call(["which", "make"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        == 0
    ):
    ...
    if sys.platform == "darwin":
        ...
        console.print(
            "[red]Install build-essential manually from https://packages.ubuntu.com/search?suite=all&section=all&arch=amd64&searchon=names&keywords=build-essential[/red]"
        )
        sys.exit(1)
...
def install_cfssl_with_git():
    ...
    if (
        subprocess.call(["which", "cfssl"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        == 0
    ):
